...
class ZeroOrderExtrapolation(object):
    ...
    def __call__(self,xo):
        ...
        if isinstance( xo, collections.Iterable ):
            # the output shape
            ...
            i = self.__x.searchsorted(xo,side = 'right') - 1
            
            # if below range, return first item
            i = max(i,0)
            
            # if abov range, return last item
            i = min(i,len(self.__y))
